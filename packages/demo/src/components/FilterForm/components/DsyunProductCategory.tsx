...
import { getDSYunCategoryList,getDSYunProductCategoryById } from '@/services/product';
import { useControllableValue } from 'ahooks'
import { FilterContext } from '../FilterContext'

const DsyunProductCategory = React.memo(React.forwardRef((props: any, ref) => {
    ...
    const bindData = useCallback((data) => {
        let initialValue=restProps.value
        if(Array.isArray(initialValue)){
          return Promise.all(restProps.value.map((id:any)=>fetchData(id))).then((datas:any[])=>{
                let current=data
    ...
    return (
      <Cascader {...restProps} loadData={loadData} options={options}>
      </Cascader>
    );
  }),
);
