...
type ModalType=[{props:any,state:ModalStateType},{
    show:(title?:any,dataItem?:any)=>void,
    close:(e?:any)=>void,
    setStateOptions:<S extends any>(options:ModalProps|((prevState:S)=>S))=>void,
...
export const useModal=(options:useModalProps):ModalType=>{
    ...
    let setInnerState=useCallback((state:any)=>{
    ...
    let onCancelHandle=useCallback((e)=>{
        setInnerState({
            title:'',
            visible:false,
            dataItem:null
    ...
    return [{props:modalProps,state:state},{show:showModal,close:onCancelHandle,setState,setStateOptions:setStateOptionsHandle}]
}