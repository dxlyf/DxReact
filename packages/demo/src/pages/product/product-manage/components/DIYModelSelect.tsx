...
let DIYModelSelect = React.memo(React.forwardRef((props:any,ref) => {
    let {onSelect}=props
    ...
    return <>
        {value!==''&&value!==undefined&&value!==undefined?<div><Tag closable onClose={onClear}>
        已选
    </Tag></div>:<div><a onClick={onSelectModel}>选择3d模型</a></div>}
    <DIYModelListModal
        visible={visible}
        onCancel={()=>{
            setVisible(false)
