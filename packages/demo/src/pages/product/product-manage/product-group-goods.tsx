...
import GroupProductListModal,{GroupProductList} from './components/GroupProductList'
...
let GoodsManage:React.FC<{}&ConnectProps<any>>=({match})=>{
    ...
    const columns=useMemo<RichTableColumnType<any>[]>(()=>[
        {
            ...
            },
            {
                title:"操作",
                render:(record)=>{
                    return <Link to={`/product/product-manage/list/edit/${record.id}`}>编辑</Link>
                }
            }
    ...
    const onComfirmGroupProduct=useCallback((products:any[])=>{
        if(products.length<=0){
            return
        }
        productService.addGroupProductList({
            groupId:groupId,
    ...
    return <Space className="m-edit-wrapper" direction="vertical">
        ...
        <Card title="基本信息" bordered={false} className="m-card">
                <Descriptions>
                    <Descriptions.Item label="商品分组名称">{groupName}</Descriptions.Item>
                </Descriptions>
         </Card>
        ...
        <GroupProductListModal visible={productModalVisible} onOk={onComfirmGroupProduct} onCancel={()=>{
            setProductModalVisible(false)
        }} dataItem={{id:groupId,relType:1}}></GroupProductListModal>
    </Space>
}
