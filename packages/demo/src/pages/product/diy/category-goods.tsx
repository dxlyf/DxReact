...
let GoodsManage: React.FC<{} & ConnectProps<any>> = ({ match }) => {
    ...
    const onDelete = useCallback((id) => {
        Modal.confirm({
          ...
          onOk: () => {
            diyService.deleteCategoryProductRef({
              id: id
            }).then(() => {
    ...
    const columns = useMemo<RichTableColumnType<any>[]>(() => [
        ...
        },
        {
            ...
            render(url){
                return url?<ImageView src={url}></ImageView>:'无预览图，请及时编辑商品DIY配置信息，否则将在用户侧隐藏'
            }
        }, 
        {
            title: "操作",
    ...
    const [groupProductModalVisible,setGroupProductModalVisible]=useState(false)
    const onComfirmProduct=useCallback((rows)=>{
        ...
        diyService.addCategoryProductRef({
            diyCategoryId:categoryId,
    ...
    const onComfirmGroupProduct=useCallback((rows)=>{
        ...
        diyService.addCategoryGroupProductRef({
    ...
    return <Space className="m-edit-wrapper" direction="vertical">
        <Card title="基本信息" bordered={false} className="m-card">
            <FilterForm fields={fields} onQuery={showList} autoBind searchProps={{ span: 24 }}>
        ...
        <ProductListModal dataItem={{id:categoryId,relType:2}} visible={productModalVisible} onOk={onComfirmProduct} onCancel={()=>{
            setProductModalVisible(false)
        }} ></ProductListModal>
        <GroupProductListModal visible={groupProductModalVisible} onOk={onComfirmGroupProduct} onCancel={()=>{
            setGroupProductModalVisible(false)
