 ...
 */
 ...
 import { Tabs, Button, Card, Space, message, Modal,Typography,Switch,Form,Input } from 'antd';
 ...
 let DIYCategory: ConnectRC<any> = ({ history,match }) => {
       ...
       onOk:()=>{
            modalForm.validateFields().then((values:any)=>{
                ...
                if(!modalState.dataItem){
                    ...
                    diyService.updateThemeCategory({
   ...
   const onDeleteHandle=useCallback((record)=>{
    Modal.confirm({
        ...
        onOk: () => {
          diyService.deleteThemeCategory({
            id:record.id
          }).then(() => {
   ...
   const onUpSort=useCallback((record,index)=>{
      let targetId=dataSource[index-1].id
      let moveId=record.id
      diyService.updateThemeCategorySort({
        coverI:moveId,
        coverII:targetId
      }).then(()=>{
        showList(true)
   ...
   const onDownSort=useCallback((record,index)=>{
    let targetId=dataSource[index+1].id
    let moveId=record.id
    diyService.updateThemeCategorySort({
      coverI:moveId,
     ...
     () => [
       ...
       },
       {
         ...
         render(value: number,record:any,index:number) {
            return <Space>
            <ArrowUpOutlined onClick={onUpSort.bind(null,record,index)} style={{color:"#1890ff",cursor:"pointer",visibility:index==0?"hidden":"visible"}}></ArrowUpOutlined>
            <ArrowDownOutlined onClick={onDownSort.bind(null,record,index)} style={{color:"#1890ff",cursor:"pointer",visibility:index+1>=dataSource.length?"hidden":"visible"}}></ArrowDownOutlined></Space>
         ...
         render: (record: any) => {
           return (
             <Space>
               ...
               <Link to={`/product/diy/theme/category/${themeId}/goods/${record.id}`}>商品管理</Link>
             </Space>
           );
         },
       },
     ],
     [showModal,onDeleteHandle,onUpdateHandle,themeId,dataSource,onUpSort,onDownSort],
   );
     ...
     <Space direction="vertical" className="m-list-wrapper">
       ...
       <Modal {...modal.props}>
           <Form form={modalForm} preserve={false} wrapperCol={{span:18}} labelCol={{span:6}}>
               <Form.Item label="分类名称" name="name" initialValue={get(modal.state.dataItem, 'name')} rules={[{
                   required:true,
                   message:"请输入分类名称！",
                   whitespace:true
