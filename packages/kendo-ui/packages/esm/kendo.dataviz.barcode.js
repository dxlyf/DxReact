...
import { defined, TextBox, Box, getSpacing } from '@progress/kendo-charts';
...
(function($, undefined$1) {
    ...
    var Barcode = Widget.extend({
        init: function(element, options) {
             ...
             that.surface = drawing.Surface.create(that.surfaceWrap[0], {
        ...
        _render: function() {
            var that = this,
                ...
                textMargin = getSpacing(textOptions.margin),
                size = that._getSize(),
                border = options.border || {},
                encoding = that.encoding,
                contentBox = new Box(0, 0, size.width, size.height).unpad(border.width).unpad(options.padding),
            ...
            if (textOptions.visible) {
                textHeight = drawing.util.measureText(value, { font: textOptions.font }).height;
        ...
        _getSize: function() {
            var that = this,
                ...
                size = new geometry.Size(DEFAULT_WIDTH, DEFAULT_HEIGHT);
        ...
        _getBands: function(pattern, baseUnit) {
            ...
            for (var i = 0; i < pattern.length; i++) {
                ...
                if (i % 2) {
                    var rect = geometry.Rect.fromPoints(
                        new geometry.Point(position, item.y1 + contentBox.y1),
                        new geometry.Point(position + step, item.y2 + contentBox.y1)
                    );
        ...
        _getBackground: function(size) {
            ...
            var path = drawing.Path.fromRect(box.toRect(), {
                fill: {
