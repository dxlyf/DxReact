...
export const defaultRequestConfig:Partial<RequestConfig<any>>={
    baseURL:app.config.AXIOS_API_BASE_URL,
...
// 状态控制
class GlobalLoading {
    private handler: ReturnType<typeof Toast.info> | null = null
    ...
    if (error.response) {
        ...
        if (error.isBusinessError&&!skipBusinessErrorHandler) {
           ...
           // const data=response.data[retDataField as keyof ResponseData<any>]
            ...
            if(response.status === HttpStatusCode.Unauthorized) {
                ...
                Toast.info({ icon: "fail", content: "请先登录!" });
            }
        }
        ...
        if (error.code && error.code === "ECONNABORTED") {
          ...
          if (error.code && error.code === "ERR_NETWORK") {
            Toast.info({ icon: "fail", content: '网络连接出现问题!' });
            return Promise.reject(error);
          }
        Toast.info({
            icon: 'fail',
