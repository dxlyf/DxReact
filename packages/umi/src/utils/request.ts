import type { ResponseError as ResponseErrorType, RequestOptionsInit } from 'umi-request'
...
request.use(async function (ctx, next) {
    ...
    // @ts-expect-error
    throw new ResponseError(ctx.res.response, '业务错误', ctx.res.data, ctx.req, 'ResponseError');
})
