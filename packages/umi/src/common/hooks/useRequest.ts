...
interface useRequestConfig<DataItem> {
    autoBind?:boolean,
    service?: (params: object, ctx: RequestContext<DataItem>) => Promise<ServiceResult<DataItem>|any>,
...
type RequestContext<DataItem> = {
    ...
    update:()=>void,
    tableProps:any
} & Omit<useRequestConfig<DataItem>, 'isPagination' | 'pageIndexField' | 'pageSizeField'>
export default function useRequest<T>(options: useRequestConfig<T>,deeps:any[]=[]):[RequestContext<T>,{query:(...args:any[])=>void,onTableChange:(arg:any)=>void}] {
    let { pageIndexField = "pageNum", pageSizeField = "pageSize",autoBind=false, service,transform, onQuery, params = {}, pagination = {}, isPagination = true } = options
    ...
    const onQueryHandle = useCallback((parmas = {}, options = false) => {
        ...
        let newParmas = {
            ...(isPagination ? { [pageIndexField]: 1, [pageSizeField]: context.pagination?.pageSize } : {}),
        ...
        if (context.service) {
            ...
            context.update()
            context.service(newParmas, context).then(d => {
    ...
    const onTableChange = useCallback(({ current, pageSize }) => {
        onQueryHandle({
