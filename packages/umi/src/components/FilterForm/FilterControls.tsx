...
import { Input, Form,Button,Select,Dropdown,Menu,Row,Col,FormInstance} from 'antd'
...
export interface FilterFormField {
    ...
    order?:number,
    skipQuery?:boolean,
    compose?:(filterParams:any,formValues:any,field:FilterRenderField)=>any
...
export type  FilterRenderField={fieldIndex?:number,key:string|number} & FilterFormField
...
create('list',{
    ...
    render(field){
        return <Select {...field.props}>{field.data?field.data.map((d:any)=><Select.Option value={d.value} key={d.value}>{d.text}</Select.Option>):field.props.children}</Select>
    }
})
