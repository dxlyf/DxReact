...
const setModel = (val: UploadFile[]) => {
    ...
    if (isMultipleFiles.value) {
        ...
        if (!props.autoUpload) {
            ...
            checkImageDimensions(val[0]?.url||'')
        }
    }
}
...
const beforeUpload: TdUploadProps['beforeUpload'] = async (file) => {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (props.extension?.length > 0 && ext && !props.extension?.some(d => ext === d.toLowerCase())) {
            msg += `/格式`
        }
...
<template>
    <div>
                ...
                <li v-for="(file, index) in allDisplayFiles" :key="index"
                    ...
                    <div v-if="!['progress', 'fail'].includes(file.status)" class="w-full h-full">
                     ...
                     <div class=" flex justify-center gap-2 w-full items-center">
                            <span class="">
                                <t-image-viewer :default-index="index"
                                    :images="files.map(file => file.url || file.raw)">
        ...
        <t-upload ref="uploadRef" :files="files" v-bind="uploadProps"
            :class="[props.draggable ? 'size-[110px] f-upload-draggable' : '']">
