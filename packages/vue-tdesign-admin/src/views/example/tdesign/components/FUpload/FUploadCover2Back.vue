...
const modelRawFile = defineModel<File>('rawfile')
...
const showUpload=computed(()=>{
  ...
  return !showProgress&&!showImage.value
})
...
const handleSuccess:UploadProps['onSuccess']=(ctx)=>{
 // imageUrl.value=ctx.response.url
  ...
  modelRawFile.value=ctx.file.raw
...
const beforeUpload:UploadProps['beforeUpload']=async (file)=>{
   ...
   return new Promise(async (resolve,reject)=>{
     ...
     // file.thumbnailUrl=img.src
      ...
      state.showProgress=props.autoUpload
      modelRawFile.value=file.raw
      resolve(true)
   })
}
const handleDelete=()=>{
