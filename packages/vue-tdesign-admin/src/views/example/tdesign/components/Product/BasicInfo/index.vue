...
<template>
    <t-form class="w-full" @submit="handleSubmit" :data="formData" :rules="rules" label-align="top" layout='vertical'>
        <t-collapse :default-expand-all="true" :expand-mutex="false" :expand-on-row-click="true" expand-icon-placement="right" borderless>
                    ...
                    <t-row :gutter="12" class="w-full">
                        <t-col :span="6">
                            <t-form-item name="videoGroupSlug" :label-width="0">
                                <t-select v-model="formData.videoGroupSlug" placeholder="请选择视频组Slug" :options="videoGroupSlugOptions" />
                            </t-form-item>
                        </t-col>
                        <t-col :span="6">
            ...
            <t-collapse-panel value="2">
                ...
                <t-form-item label="Status(发布状态)" name="status" >
                    <t-select v-model="formData.status" :options="[{label:'Publish',value:'Publish'},{label:'Draft',value:'Draft'}]" placeholder="请输入发布状态"  />
                    <template #help v-if="showEnglishDefaultValue('status')">
                        <div>当前字段内容复用英文</div>
                    </template>
                </t-form-item>
                <t-form-item label="Publish at(发布时间)" name="publish_at" >
                    <t-date-picker :enable-time-picker="true" v-model="formData.publish_at" placeholder="请输入发布时间" :maxlength="255" >
                    </t-date-picker>
                    <template #tips>
                ...
                <t-form-item label="Accessory Link(配件链接)">
                    <div class="grid grid-cols-[200px_1fr] gap-x-2 w-full">
                        <t-form-item :label-width="0" name="accessoryLinkType" >
                            <t-select :options="[{label:'相对链接',value:'相对链接'},{label:'绝对链接',value:'绝对链接'}]" placeholder="Select an Option" v-model="formData.accessoryLinkType"  />
                        </t-form-item>
