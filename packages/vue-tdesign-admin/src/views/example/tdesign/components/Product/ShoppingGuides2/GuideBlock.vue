...
const handleHighlightJsonChange = (val: string) => {
    formData.value.highlightJson = val
    if (isJsonArray(val)) {
        formData.value.highlight = JSON.parse(val)
    }
}
watch(()=>formData.value.highlight, (val) => {
    if(Array.isArray(val)&&JSON.stringify(val)!==formData.value.highlightJson){
        formData.value.highlightJson = JSON.stringify(val)
    }
},{deep:true})
...
<template>
    <t-form-item :rules="[{ required: true, message: '请上传title' }]" label="Title" :name="`${prefix}.title`">
        <UploadCover :skip-load-check="false" :load-check-image-config="{ type: 'svg', width: 362, height: 32 }"
            :extension="['svg', 'png']" tips="上传图片格式：SVG,尺寸：362x32" subtitle="支持上传扩展名：PNG、JPG、GIF、SVG"
            :limit="{ size: 2, width: 362, height: 32 }" v-model="formData.title"></UploadCover>
    </t-form-item>
    <t-form-item :rules="[{ required: true, message: '请上传cover' }]" label="Cover" :name="`${prefix}.cover`">
        <UploadCover :limit="{ size: 2, width: 720, height: 720 }" tips="上传图片格式：SVG,尺寸：362x32"
            subtitle="支持上传扩展名：PNG、JPG、GIF、SVG" v-model="formData.cover"></UploadCover>
    </t-form-item>
    <t-form-item label="Highlight">
        <div class="flex flex-col w-full">
            <div class="grid grid-cols-[1fr_200px] gap-x-2 w-full">
                <div>
                    <t-form-item :label-width="0" :name="`${prefix}.highlightJson`" :rules="[{
                        validator(val, { formData }) {
                            if (typeof val === 'string' && val.trim() !== '' && !isJsonArray(val)) {
                                return {
                                    result: false,
                                    message: '必须是JSON数组格式'
                                }
                        ...
                        <t-input  :value="formData.highlightJson" @change="handleHighlightJsonChange">
                        </t-input>
                    </t-form-item>
                    <div class="h-6"></div>
                </div>
            ...
            <div class="grid grid-cols-[1fr_200px] mt-2 gap-2">
                <template v-for="(item, index) in formData.highlight" :key="index">
                    ...
                    <div>
                        <t-button v-if="visibleDelHighlight" theme="danger"
                            @click="formData.highlight.splice(index, 1)">Delete This Highlight</t-button>
                    </div>
                </template>
            </div>
        </div>
    ...
    <div class="grid grid-cols-2 gap-x-2">
        <div>
            <t-form-item :rules="[{ required: true, message: '请输入title' }]" label="Primary Button Title"
                :name="`${prefix}.primaryButtonTitle`">
                <t-input v-model="formData.primaryButtonTitle" placeholder="Primary Button title"></t-input>
            </t-form-item>
            <div></div>
        </div>
        <div>
            <t-form-item :rules="[{ required: true, message: '请输入link' }]" label="Primary Link title"
                :name="`${prefix}.primaryLinkTitle`">
                <t-input v-model="formData.primaryLinkTitle" placeholder="Primary Button link"></t-input>
            </t-form-item>
            <div></div>
        </div>
        <div>
            <t-form-item :rules="[{ required: true, message: '请选择link type' }]" label="Primary Link Type"
                :name="`${prefix}.primaryLinkType`">
                <t-select placeholder="请选择link type" v-model="formData.primaryLinkType"
                    :options="LinkTypeOptions"></t-select>
            </t-form-item>
            <div></div>
        </div>
        <div>
                    ...
                    <div class="grid grid-cols-3 gap-x-2">
                        <div>
                            <t-form-item :name="`${prefix}.secondaryButton[${index}].title`">
                                <t-input v-model="formData.secondaryButton[index].title"
                                    placeholder="Secondary Button Title"></t-input>
                            </t-form-item>
                        </div>
                        <div>
                            <t-form-item :name="`${prefix}.secondaryButton[${index}].link`">
                                <t-input v-model="formData.secondaryButton[index].link"
                                    placeholder="Secondary Button Link"></t-input>
                            </t-form-item>
                        </div>
                        <div>
                            <t-form-item :name="`${prefix}.secondaryButton[${index}].linkType`">
                                <t-select v-model="formData.secondaryButton[index].linkType"
                                    placeholder="Select an Option" :options="LinkTypeOptions"></t-select>
                            </t-form-item>
                        </div>
                    </div>
                </div>
