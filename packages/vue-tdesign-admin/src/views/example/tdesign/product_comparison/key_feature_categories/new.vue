...
const [drawerProps, { open: openDrawer, close: closeDrawer }] = useDrawer(() => ({
    header: '编辑特性',
    size: '40%',
    footer: true,
...
const removeFeature = (index: number) => {
    const confirmDia = DialogPlugin.confirm({
        header: '确认删除',
        body: `确定要删除特性「${formData.features[index]?.title || '未设置'}」吗？`,
        confirmBtn: '删除',
        onConfirm: () => {
        ...
        <t-form :data="formData" :label-width="140" @submit="handleSubmit" class="w-full" label-align="top">
            <t-collapse ref="collapseRef" v-model="expandedPanel" :expand-on-row-click="false"
                ...
                <t-collapse-panel value="features">
                    ...
                    <div class="flex flex-col border border-gray-200 rounded-md overflow-hidden">
                        <div v-for="(item, index) in formData.features" :key="item.id"
                            ...
                            <t-tag variant="light" theme="default" size="small">{{ item.type === 'image' ? '图片' :
                                '文本'
                                }}</t-tag>
