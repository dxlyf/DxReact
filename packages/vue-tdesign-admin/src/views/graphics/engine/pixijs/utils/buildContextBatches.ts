...
[buildRectangle, buildPolygon, buildTriangle, buildCircle, buildEllipse, buildRoundedRectangle].forEach((build) =>
{
    shapeBuilders[build.extension.name] = build;
});
 ...
 * @internal
 */
export function buildContextBatches(context: any, gpuContext: any)
{
...
export function addTextureToGeometryData(
    data: any,
    batches: any[],
    geometryData: {
        vertices: number[];
...
export function addShapePathToGeometryData(
    shapePath: ShapePath,
    style: any,
    isStroke: boolean,
    batches: any[],
...
)
{
    ...
    shapePath.shapePrimitives.forEach(({ shape, transform: matrix, holes }) =>
    {
        ...
        const indexOffset = indices.length;
        const vertOffset = vertices.length / 2;
        let topology: string = 'triangle-list';
...
export function getHoleArrays(holePrimitives: ShapePrimitiveWithHoles[])
{
    const holeArrays = [];
