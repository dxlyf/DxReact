/**
 * CPU 光栅化系统 —— 帧缓冲。
 * 颜色缓冲（RGBA8）+ 深度缓冲（Float32，[0,1]）+ 模板缓冲（Uint8，0-255），模拟 WebGL 默认帧缓冲。
 */
...
export class CPUFramebuffer implements RasterImage {
    ...
    constructor(width: number, height: number) {
        ...
        this._stencil = new Uint8Array(this._width * this._height)
    }
    ...
    /** 清空模板缓冲（默认 0） */
    clearStencil(value = 0): void {
        this._stencil.fill(value & 0xff)
    }
    ...
    readStencil(x: number, y: number): number {
        ...
        return this._stencil[y * this._width + x]
    }

    /** 写入模板像素（0-255） */
    writeStencil(x: number, y: number, value: number): void {
        if (x < 0 || x >= this._width || y < 0 || y >= this._height) return
        this._stencil[y * this._width + x] = value & 0xff
    }
