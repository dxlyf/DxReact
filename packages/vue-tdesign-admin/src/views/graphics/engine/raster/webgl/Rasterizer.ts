...
import type { BlendFactors, DepthFunc, DrawMode, FragmentInput, ShaderProgram, StencilOp, Uniforms } from './types'
...
export interface RasterizerOptions {
    ...
    /** 模板参考值（ref） */
    stencilRef: number
    /** 模板读掩码（ref 与缓冲值都先 & mask 再比较） */
    stencilReadMask: number
    /** 模板写掩码（只允许写 mask 为 1 的位） */
    ...
    /** 是否启用裁剪测试（gl.SCISSOR_TEST） */
    scissorTest: boolean
    /** 裁剪矩形（帧缓冲内部左上原点坐标，右/下边界开区间） */
...
function inside(p: Vec4, axis: string, sign: number): boolean {
    ...
    // 带 epsilon：避免顶点恰好在 NDC 边界（如 x=1）时浮点误差导致被误裁剪
...
/**
 * 计算模板操作后的新值（对应 gl.stencilOp 的 KEEP/ZERO/REPLACE/INCR/DECR/INVERT/INCR_WRAP/DECR_WRAP）。
 * 注意：与真实 WebGL 一致，写入时按 writeMask 只更新允许的位。
 */
function stencilOpValue(op: StencilOp, current: number, ref: number, writeMask: number): number {
    let value: number
    switch (op) {
        case 'keep': value = current; break
        case 'zero': value = 0; break
        case 'replace': value = ref & 0xff; break
        case 'incr': value = Math.min(255, current + 1); break // 饱和递增
        case 'decr': value = Math.max(0, current - 1); break // 饱和递减
        case 'invert': value = ~current & 0xff; break
        case 'incrWrap': value = (current + 1) & 0xff; break // 回绕递增
        case 'decrWrap': value = (current - 1) & 0xff; break // 回绕递减（-1 & 0xff = 255）
    }
...
export class Rasterizer {
    ...
    /** 本次 draw 实际写入帧缓冲的片元数（遮挡查询 ANY_SAMPLES_PASSED 用） */
    private writtenPixels = 0
     ...
     */
    draw(program: ShaderProgram, uniforms: Uniforms, vertices: ClipVertex[], indices: Uint16Array | Uint32Array | null): number {
    ...
    private processFragment(
        ...
        const { framebuffer, depthTest, depthWrite, depthFunc, stencilTest, blend: doBlend, scissorTest, scissorRect } = this.options

        // 帧缓冲边界：视口可超出绘制缓冲，越界片元直接丢弃（同 GL 行为）
        if (x < 0 || x >= framebuffer.width || y < 0 || y >= framebuffer.height) return

        // 裁剪测试：区外片元直接丢弃（在任何缓冲写入之前，同 WebGL 的早期阶段）
        if (scissorTest) {
            if (x < scissorRect.x || x >= scissorRect.x + scissorRect.width || y < scissorRect.y || y >= scissorRect.y + scissorRect.height) return
        }
        ...
        // 模板测试（WebGL 顺序：模板先于深度）
        if (stencilTest) {
            const current = framebuffer.readStencil(x, y)
            const ref = this.options.stencilRef & this.options.stencilReadMask
            const stencil = current & this.options.stencilReadMask
            if (!depthTestPass(this.options.stencilFunc, ref, stencil)) {
                framebuffer.writeStencil(x, y, stencilOpValue(this.options.stencilOpFail, current, this.options.stencilRef, this.options.stencilWriteMask))
                return
            }
        }
        ...
        if (depthTest) {
            ...
            if (!depthTestPass(depthFunc, z, depth)) {
                if (stencilTest) {
                    framebuffer.writeStencil(x, y, stencilOpValue(this.options.stencilOpZFail, framebuffer.readStencil(x, y), this.options.stencilRef, this.options.stencilWriteMask))
                }
                return
            }
        }

        // 模板+深度都通过：应用 zpass 操作，然后写入颜色与深度
        if (stencilTest) {
            framebuffer.writeStencil(x, y, stencilOpValue(this.options.stencilOpZPass, framebuffer.readStencil(x, y), this.options.stencilRef, this.options.stencilWriteMask))
        }
