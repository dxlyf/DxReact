...
onMounted(async () => {
    ...
    graphicsPath.roundRect(100, 100, 100, 100,10)
 ...
 //  const shapes=shapePath.toShapes(false)
    ...
    gl.bufferData(gl.ARRAY_BUFFER,new Float32Array(geometryData.vertices),gl.STATIC_DRAW)

    gl.enableVertexAttribArray(0)
    gl.vertexAttribPointer(0,2,gl.FLOAT,false,4*2,0)

    const indecisBuffer = gl.createBuffer()
    gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER,indecisBuffer)
    gl.bufferData(gl.ELEMENT_ARRAY_BUFFER,new Int16Array(geometryData.indices),gl.STATIC_DRAW)
    gl.bindVertexArray(null)
   

    let projectMatrix = glMatrix.mat3.create()
    //  glMatrix.mat3.set(projectMatrix,1,0,0,1,0,0,0,0,1,0)
    glMatrix.mat3.projection(projectMatrix, gl.drawingBufferWidth, gl.drawingBufferHeight)

   
    gl.uniform3fv(gl.getUniformLocation(program,'uColor'), new Float32Array([1, 0, 0]))
    gl.uniformMatrix3fv(gl.getUniformLocation(program,'projectMatrix'),false, projectMatrix)
    

    gl.bindVertexArray(vao)
    gl.cullFace(gl.FRONT_AND_BACK)
    gl.drawElements(gl.TRIANGLES, geometryData.indices.length, gl.UNSIGNED_SHORT, 0)
