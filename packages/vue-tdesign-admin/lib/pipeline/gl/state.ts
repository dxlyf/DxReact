...
abstract class Value<Context, T> {
    ...
    abstract init(ctx:Context):void
...
abstract class GLValue<T> extends Value<GLState,T> {
    ...
    override init(ctx:GLState):void{
        this.gl=ctx.gl
    }
}
...
export type ViewportType=[x:number,y:number,width:number,height:number]
class Viewport extends GLValue<ViewportType> {
    override getDefault() {
        return [0, 0, 0, 0] as ViewportType
    }
    override equals(current: ViewportType, prev: ViewportType): boolean {
        return arrayEqual(current, prev)
    }
    override update(value: ViewportType): void {
...
export class GLState {
    ...
    resetState(){
        ...
        this.bindRenderbuffer.setDefault();
        this.bindTexture.setDefault();
        this.bindElementBuffer.setDefault();
        this.bindVertexArray.setDefault();
        this.bindVertexBuffer.setDefault();
        this.activeTextureUnit.setDefault();
        this.scissorTest.setDefault();
        this.scissor.setDefault();
        this.program.setDefault();
        this.viewport.setDefault();
        this.pixelStoreUnpackAlignment.setDefault();
